...
// and show text generated and the 'copy-text' and 'back-to-home' buttons
  ...
  sentence.click(function(){
    ...
    const randomRickQuote = () => rickLines[Math.floor(Math.random() * rickLines.length)];
    const randomMortyQuote = () => mortyLines[Math.floor(Math.random() * mortyLines.length)]
    let num = Math.floor(Math.random() * (8 - 3 + 1) + 3)

    const randomParagraph = () => {
        let num = Math.floor(Math.random() * (10 - 3 + 1) + 3), rickLines = '';

        for (var i = 0; i < num; i++) {
          rickLines += (randomRickQuote() + ' ');
          console.log([rickLines]);
        }
        return rickLines;
}

let loremGenerator = function(num) {
  let ipsum = [];
  while (ipsum.length < num) {
    ipsum.push(randomParagraph());
